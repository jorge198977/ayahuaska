...
$fecha = date("H:i:s");
$sqlhora = "select * from sheol2.horahappy where idhorahappy = 1";
...
if($ret == 1){
  if(isset($_POST['parametros'])){
    ...
    $sql = "select * from sheol2.familias where nombre like '%".$dato."%' order by nombre asc";
  }
  else{
    $sql = "select * from sheol2.familias order by nombre asc";  
  }
}
else{
  if(isset($_POST['parametros'])){
    $dato = $_POST['parametros'];
    $sql = "select * from sheol2.familias where nombre like '%".$dato."%' and nombre not like '%HAPPY%' order by nombre asc";
  }
  else{
    $sql = "select * from sheol2.familias where nombre not like '%HAPPY%' order by nombre asc";  
  }
}
