...
class Dte
{
    ...
    {
        if (class_exists('Extra\Sii\Dte\VerificadorDatos')) {
            if (!Extra\Sii\Dte\VerificadorDatos::check($this->getDatos())) {
                return false;
            }
        }
        return true;
     ...
     * @version 2015-10-24
     */
    public function getEstado(FirmaElectronica $Firma)
    {
        // solicitar token
        $token = Sii\Autenticacion::getToken($Firma);
        if (!$token) {
            return false;
        }
     ...
     * @version 2016-08-05
     */
    public function getEstadoAvanzado(FirmaElectronica $Firma)
    {
        // solicitar token
        $token = Sii\Autenticacion::getToken($Firma);
        if (!$token) {
     ...
     * @version 2017-08-29
     */
    public function getUltimaAccionRCV(FirmaElectronica $Firma)
    {
        list($emisor_rut, $emisor_dv) = explode('-', $this->getEmisor());
        $RCV = new Sii\RegistroCompraVenta($Firma);
        try {
