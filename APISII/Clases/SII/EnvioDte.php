...
require_once 'Base/Envio.php';

class EnvioDte extends Envio
{
    ...
    public function agregar(Dte $DTE)
    {
        ...
        if (isset($this->dtes[$this->config['DTE_max'][$this->tipo]-1])) {
            Log::write(
                Estado::ENVIODTE_DTE_MAX,
                Estado::get(Estado::ENVIODTE_DTE_MAX, $this->config['DTE_max'][$this->tipo])
            );
            return false;
        }
        $this->dtes[] = $DTE;
    ...
    public function setCaratula(array $caratula)
    {
        ...
        if (!isset($this->dtes[0])) {
            Log::write(
                Estado::ENVIODTE_FALTA_DTE,
                Estado::get(Estado::ENVIODTE_FALTA_DTE)
            );
            return false;
        }
        ...
        if (isset($SubTotDTE[$this->config['SubTotDTE_max'][$this->tipo]])) {
            Log::write(
                Estado::ENVIODTE_TIPO_DTE_MAX,
                Estado::get(Estado::ENVIODTE_TIPO_DTE_MAX, $this->config['SubTotDTE_max'][$this->tipo])
            );
            return false;
        }
        // generar caratula
    ...
    public function getDocumentos($c14n = true)
    {
        ...
        if (!$this->dtes) {
            ...
            if (!$this->xml) {
                Log::write(
                    Estado::ENVIODTE_GETDOCUMENTOS_FALTA_XML,
                    Estado::get(Estado::ENVIODTE_GETDOCUMENTOS_FALTA_XML)
                );
                return false;
            }
