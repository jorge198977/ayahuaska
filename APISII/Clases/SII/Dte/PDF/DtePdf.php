...
include('../../APISII/Clases/SII/Dte/Base/DteImpreso.php');
require_once '../../APISII/Clases/Chile.php';
require_once '../../APISII/Clases/Aduana.php';

class DtePdf extends PDF
{
    ...
    {
        parent::__construct();
        $this->SetTitle('Documento Tributario Electrónico (DTE)');
     ...
     * @version 2018-06-15
     */
    protected function agregarEmisor(array $emisor, $x = 12, $y = 15, $w = 75, $w_img = 30, $font_size = null, array $color = null)
    {
        // logo del documento
        if (isset($this->logo)) {
            $this->Image(
        ...
        $this->setFont('', 'B', $font_size ? $font_size : 2);
        ...
        $this->setFont('', 'B', $font_size ? $font_size : 1);
        ...
        $ciudad = !empty($emisor['CiudadOrigen']) ? $emisor['CiudadOrigen'] : Chile::getCiudad($comuna);
    ...
    protected function agregarDatosEmision($IdDoc, $CdgVendedor, $x = 10, $offset = 22, $mostrar_dia = true)
    {
        ...
        // papel contínuo
            ...
            // fecha de emisión
            $this->setFont('', 'B', null);
            $this->Texto('Emisión', $x+7);
            $this->Texto(':', $x+$offset+7);
            $this->setFont('', '', null);
            $this->MultiTexto($this->date($IdDoc['FchEmis'], $mostrar_dia), $x+$offset+9);
            // forma de pago nacional
            if (!empty($IdDoc['FmaPago'])) {
                $this->setFont('', 'B', null);
                $this->Texto('Venta', $x);
     ...
     * @version 2019-02-13
     */
    protected function agregarReceptor(array $Encabezado, $x = 12, $offset = 22)
    {
     ...
     * @version 2016-12-13
     */
    protected function agregarDetalleContinuo($detalle, $x = 1)
    {
        ...
        $style = array('width' => 0,'color' => array(0, 0, 0));
        ...
        foreach($detalle as  &$d) {
            ...
            $this->Texto(number_format($d['PrcItem'],0,',','.'), $x+5, $this->y, ucfirst($this->detalle_cols['PrcItem']['align'][0]), $this->detalle_cols['PrcItem']['width']);
            $this->Texto($this->num($d['QtyItem']), $x+30, $this->y, ucfirst($this->detalle_cols['QtyItem']['align'][0]), $this->detalle_cols['QtyItem']['width']);
            $this->Texto($this->num($d['MontoItem']), $x+36, $this->y, ucfirst($this->detalle_cols['MontoItem']['align'][0]), $this->detalle_cols['MontoItem']['width']);
        }
     ...
     * @version 2017-10-05
     */
    protected function agregarTotales(array $totales, $otra_moneda, $y = 180, $x = 145, $offset = 25)
    {
        ...
        foreach ($totales as $key => $total) {
            if ($total!==false and isset($glosas[$key])) {
                ...
                if (!$this->cedible or $this->papelContinuo) {
                    $this->Texto($glosas[$key].' :', $x, null, 'R', 20);
                    $this->Texto($this->num($total), $x+$offset, $y, 'R', 20);
                    $this->Ln();
                } else {
                    $this->MultiTexto($glosas[$key].' :', $x, null, 'R', 20);
                    $y_new = $this->GetY();
                    $this->Texto($this->num($total), $x+$offset, $y, 'R', 20);
                    $this->SetY($y_new);
                }
     ...
     * @version 2018-04-12
     */
    protected function agregarTimbre($timbre, $x_timbre = 1, $x = 1, $y = 190, $w = 20, $font_size = 5)
    {   
