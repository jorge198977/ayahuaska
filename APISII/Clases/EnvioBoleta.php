        ...
        if ($resp===false or (string)$respon->xpath('/SII:RESPUESTA/SII:RESP_HDR/ESTADO')[0]!=='00') {
            Log::write(
               Estado::AUTH_ERROR_SEMILLA,
                Estado::get(\sasco\LibreDTE\Estado::AUTH_ERROR_SEMILLA)
            );
            return false;
        }
...
/**
    ...
    {   
        ...
        do {
            $file =$boleta_ubic.'/dte_'.md5(microtime().$token.$dte).'.'.($gzip?'gz':'xml');
        ...
        $curl = curl_init();
        $header = [
            'User-Agent: Mozilla/4.0 (compatible; PROG 1.0; CABLECOLOR)',
            'Referer: https://www.cablecolor.cl',
            'Cookie: TOKEN='.$token,
        ];
    ...
    public static function consultaBoleta($empresa, $Firma, $track =0, $retry = 1)
    {   

        $token=self::getToken($Firma);
        // definir datos que se usarán en el envío
        list($rutSender, $dvSender) = explode('-', str_replace('.', '', $empresa));
        ...
        //tipo de envio para boletas
        //$url = $this->AMBIENTE.'boleta.electronica.envio/'.$rutSender.'-'. $dvSender.'-'.$track.'';
        //$url='https://apicert.sii.cl/recursos/v1/boleta.electronica.envio/'.$rutSender.'-'. $dvSender.'-'.$track.'';
        //PRODUCCION
        $url='https://api.sii.cl/recursos/v1/boleta.electronica.envio/'.$rutSender.'-'. $dvSender.'-'.$track.'';
        //$url = 'https://apicert.sii.cl/recursos/v1/globales/boleta.electronica.tipo';
        //$url ='https://pangal.sii.cl/recursos/v1/'.'boleta.electronica.envio';
    ...
    public static function consultaEstadoBoleta($empresa, $Firma, $tipo, $folio, $receptor, $monto, $fecha, $retry = 1)
    {   
        ...
        list($rutRecep, $dvRecp) = explode('-', str_replace('.', '', $receptor));
        ...
        //tipo de envio para boletas
        //$url = $this->AMBIENTE.'boleta.electronica.envio/'.$rutSender.'-'. $dvSender.'-'.$track.'';
        //$url='https://apicert.sii.cl/recursos/v1/boleta.electronica/'.$rutSender.'-'. $dvSender.'-'.$tipo.'-'.$folio.'/estado?rut_receptor='.$rutRecep.'&dv_receptor='.$dvRecp.'&monto='.$monto.'&fechaEmision='.$fecha.'';
        //PRODUCCION
        $url='https://api.sii.cl/recursos/v1/boleta.electronica/'.$rutSender.'-'. $dvSender.'-'.$tipo.'-'.$folio.'/estado?rut_receptor='.$rutRecep.'&dv_receptor='.$dvRecp.'&monto='.$monto.'&fechaEmision='.$fecha.'';
        //$url = 'https://apicert.sii.cl/recursos/v1/globales/boleta.electronica.tipo';
        //$url ='https://pangal.sii.cl/recursos/v1/'.'boleta.electronica.envio';
        ...
        // verificar respuesta del envío y entregar error en caso que haya uno
        if (!$response or $response=='Error 404') {
            if (!$response) {
                Log::write(Estado::ENVIO_ERROR_CURL, Estado::get(Estado::ENVIO_ERROR_CURL, curl_error($curl)));
            }
            if ($response=='Error 500') {
